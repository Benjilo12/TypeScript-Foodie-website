import { Navlinks } from "@/app/constant/contants";
import Link from "next/link";
import React from "react";
import { CgClose } from "react-icons/cg";

//we define the props to be received
type Props = {
  showNav: boolean;
  closeNav: () => void;
};
        ...
        {Navlinks.map((link) => {
          return (
            <Link key={link.id} href={link.url}>
              <p className="text-white w-fit text-[20px] ml-12 border-b-[1.5px] pb-1 border-white sm:text-[20px]">
                {link.label}
              </p>
            </Link>
          );
