    ...
    >
      <div className="flex items-center h-full justify-between w-[90%] xl:w-[80%] mx-auto">
        ...
        <div className="flex items-center space-x-2">
          ...
          <h1 className="text-xl hidden sm:block md:text-2xl text-black font-bold dark:text-white">
            Foodies
          </h1>
        </div>
        {/* Navlinks */}
          ...
          {Navlinks.map((link) => {
            return (
              <Link
                key={link.id}
                href={link.url}
                className="text-black hover:text-emerald-500 font-bold transition-all duration-200 dark:text-white"
              >
                <p>{link.label}</p>
              </Link>
            );
        ...
        <div className="flex items-center space-x-4">
          <button className="bg-blue-500 dark:bg-white dark:text-black text-white font-bold rounded-lg hover:bg-blue-700 transition-all duration-300 cursor-pointer px-8 py-2.5 dark:hover:bg-gray-200">
            Join Now
          </button>
          {/* Theme Toggler */}
          <ThemeToggler />
